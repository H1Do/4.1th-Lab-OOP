...
namespace OOP_Lab_4_1
{
    ...
    {
        List<CCircle> circles; // Список всех окружностей
        Designer designer; 

        bool is_ctrl_pressed = false; // Состояние ctrl

        public Form1()
        {
            InitializeComponent();
            circles = new List<CCircle>(); // Определяем список окружностей
            designer = new Designer(pictureBox.Width, pictureBox.Height); // Определяем класс, отвечающий за отрисовку
        }

        private void NewCircle(int x, int y) // Создаём новый кружок, добавляем его в список всех окружностей и отрисовываем всё изображение (вместе с ним)
        {
        ...
        private void pictureBox_MouseUp(object sender, MouseEventArgs e) // Обрабатываем событие клика (Выделение или создание окружностей)
        {
            designer.Clear();
        ...
        {
            designer.DrawAll(circles); // Отрисовываем все окружности
        }
        ...
        private void Form1_KeyUp(object sender, KeyEventArgs e) // Обрабатываем событие отпускания клавиши
        {
